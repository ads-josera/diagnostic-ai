 ...
 */
/* =============================================================================
 * SALES LEADERSHIP DIAGNOSTIC AI — Configuración compartida del proyecto
 * =============================================================================
 * Este archivo SÍ se versiona en Git. Por lo tanto:
 *
 *   ⚠ NUNCA escribas aquí una API key, un secreto, un token ni una contraseña.
 *
 * Los secretos se leen exclusivamente de variables de entorno (§29, §49):
 *   - Desarrollo: .ddev/config.local.yaml → web_environment (ignorado por Git)
 *   - Staging/Producción: variables de entorno reales del servidor
 *
 * Las credenciales de base de datos las inyecta settings.ddev.php en local
 * (archivo generado por DDEV e ignorado por Git).
 * ========================================================================== */

/**
 * Directorio de configuración exportada, fuera del document root.
 */
 ...
 * Hash salt.
 *
 * En desarrollo lo provee DDEV mediante settings.ddev.php (incluido más
 * arriba). En staging y producción debe venir del entorno.
 */
...
/**
 * Secretos del módulo sales_leadership_diagnostic (§29).
 *
 * El módulo los consume mediante Settings::get(). Si alguno falta,
 * hook_requirements() lo reportará en /admin/reports/status y la
 * funcionalidad afectada fallará de forma cerrada (§13).
 */
 ...
 * Se define por entorno mediante DRUPAL_TRUSTED_HOST (regex, sin delimitadores).
 * Ejemplo: ^diagnostic\.salesbumm\.com$
 */
if ($trusted_host = getenv('DRUPAL_TRUSTED_HOST')) {
 ...
 * Load local development override configuration, if available.
 *
 * Mantener este bloque al final para que pueda sobrescribir lo anterior.
 */
